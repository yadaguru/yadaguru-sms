...
module.exports = function() {
   ...
   */
  const TEST_NUMBER = process.env.TEST_NUMBER || false;
  ...
  smsSender.sendDailySms = function() {
    logger.info(`Preparing to send SMS messages for ${PROCESS_DATE}`);
    getReminders(PROCESS_DATE).then(reminders => {
      logger.info(`Found ${reminders.length} to process`);
      ...
      processQueue(() => {
        logger.info('SMS Send process complete!');
        process.exit(0)
      });
    }).catch(error => {
      logger.error('There was a problem getting reminders from the database', error);
      process.exit(1);
    });
  };
  ...
  function processQueue(done) {
    logger.info(`SMS messages to send for ${PROCESS_DATE}: ${reminderQueue.length}`);
    ...
    sendMessage(message).then(() => {
      ...
      logger.error('There was a problem processing the messages', error);
      process.exit(1);
    });
  }
  ...
  function createMessage(reminders) {
    ...
    const reminderCount = deDupedReminders.length;
    ...
    for (let i = 0; i < reminderCount; i++) {
      ...
      message.text += deDupedReminders[i].smsMessage + '\n'
    }

    // get the 4th message (if there are only 4)
    if (reminderCount === 4) {
      message.text += deDupedReminders[4].smsMessage + '\n'
    }
    ...
    message.text += getShortlink(deDupedReminders[0].userId);
  ...
  function sendMessage(message) {
    logger.info(`Message to ${message.phoneNumber}`);
    ...
    if (TEST_NUMBER) {
      logger.warn(`Using test number. All messages will be sent to ${TEST_NUMBER}`);
